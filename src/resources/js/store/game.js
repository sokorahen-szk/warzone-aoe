...
const actions = {
    packageList({ commit }) {
        return new Promise((resolve, reject) => {
            axios.get("/api/game/package/list").then(res => {
                if (res.data && res.data.isSuccess) {
                    commit("setPackageList", {
                        $packages: res.data.body.gamePackages
                    });
                    resolve();
                } else {
    ...
    mapList({ commit }) {
        return new Promise((resolve, reject) => {
            axios.get("/api/game/map/list").then(res => {
                if (res.data && res.data.isSuccess) {
    ...
    ruleList({ commit }) {
        return new Promise((resolve, reject) => {
            axios.get("/api/game/rule/list").then(res => {
                if (res.data && res.data.isSuccess) {
    ...
    gameFinished({ commit }, payload) {
        ...
        return new Promise((resolve, reject) => {
            axios.post("/api/game/create/finished", params).then(res => {
                if (res.data && res.data.isSuccess) {
                    resolve();
                } else {
                    reject(res.data.errorMessages);
    ...
    adminGameFinished({ commit }, payload) {
        ...
        return new Promise((resolve, reject) => {
            axios
                .post(`/api/admin/game/${payload.gameRecordId}`, params)
                .then(res => {
                    if (res.data && res.data.isSuccess) {
                        resolve();
                    } else {
