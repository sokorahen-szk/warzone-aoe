...
<v-form
  ...
  lazy-validation
>
  <v-row no-gutters v-if="selectedUser.id || selectedEditType === editTypes.create">
    <v-col cols="12" class="py-0 ma-0">
      <v-row no-gutters v-if="selectedEditType === editTypes.edit">
        <v-col cols="6">
          <v-col cols="12" class="py-0 ma-0">
              <div>ユーザID</div>
          ...
          <v-col cols="12" class="py-0 ma-0">
              <div>アカウント作成日</div>
              <div class="mb-4">{{selectedUser.joinedAt}}</div>
          </v-col>
          <v-col cols="12" class="py-0 ma-0">
              <div>最終ゲーム参加日</div>
              <div class="mb-4">{{selectedUser.lastGameAt}}</div>
          </v-col>
        </v-col>
    ...
    <v-col cols="12" class="py-0 ma-0" v-if="selectedEditType === editTypes.create">
    ...
    <v-col cols="12" class="py-0 ma-0" v-if="selectedEditType === editTypes.edit">
    ...
    <v-col cols="12" class="py-0 mb-4" v-show="!isEditPassword">
    ...
    <v-col cols="12" class="py-0 mb-0" v-show="isEditPassword">
  ...
  methods: {
    ...
    show(editType, id) {
      if (this.editTypes.create == editType) {
        this.$set(this, 'selectedUser', Object.assign({}, editUserByAdminTemplate))
    ...
    update() {
      ...
      if (this.editTypes.create == this.selectedEditType) {
        ...
        .finally( () => {
          ...
          this.selectedUser = Object.assign({}, editUserByAdminTemplate)
        })
      }
      if (this.editTypes.edit == this.selectedEditType) {
        new Promise((resolve) => {
          resolve(this.updateUser({
            ...
            roleId: this.selectedUser.roleId,
            gamePackages: toString(this.selectedUser.gamePackages),
          }))
        })
        .then( (res) => {
          this.alert ={
