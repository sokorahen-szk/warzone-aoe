<template>
    <v-container>
        <CommonWithLeftColumnTemplate :device="getDeviceType">
            <template slot="container">
                <div>試合中のゲームルーム</div>
                <v-row
                    no-gutters
                    class="mt-3"
                    :class="{ 'mr-3': getDeviceType === 'pc' }"
                >
                    <Loading v-if="isLoading" :size="32" />
                    <v-col
                        cols="12"
                        v-else-if="
                            warMatchingHistories.length > 0 && !isLoading
                        "
                    >
                        <v-card
                            tile
                            v-for="warHistory in warMatchingHistories"
                            :key="warHistory.gameRecordId"
                        >
                            <v-row no-gutters class="pa-4">
                                <v-col cols="3">
                                    <v-row no-gutters>
                                        <v-col cols="12">
                                            ...
                                            ><span class="pl-2">人数</span>
                                        </v-col>
                                        <v-col cols="12" class="pl-2"
                                            >{{
                                                warHistory.playerCount
                                            }}
                                            人</v-col
                                        >
                                    </v-row>
                                ...
                                <v-col cols="4">
                                    ...
                                    ><span class="pl-2">経過時間</span>
                                    <Timer
                                        :l-datetime="warHistory.startedAt"
                                        :r-datetime="now"
                                    />
                                </v-col>
                            </v-row>
                            <v-row no-gutters class="py-3">
                                ...
                                >
                                    ...
                                    <v-list
                                        v-for="playerMemory in warHistory
                                            .playerMemories.team1"
                                        :key="playerMemory.playerMemoryId"
                                    >
                                        <v-list-item>
                                            <v-list-item-avatar>
                                                <Avator
                                                    :src="
                                                        playerMemory.avatorImage
                                                    "
                                                />
                                            </v-list-item-avatar>
                                            <v-list-item-content
                    ...
                    <v-col cols="12">
                        ...
                        <v-list class="pa-0 ma-0">
                            <v-list-item class="pa-0 ma-0" to="/news">
                                <Label color="info">お知らせ</Label>
                                <v-list-item-content>
                                    <v-list-item-title
                                        >新warzone-aoe
                                        リリースしました。</v-list-item-title
                                    >
                                    <v-list-item-subtitle
                                        >2022/03/24</v-list-item-subtitle
                                    >
                                </v-list-item-content>
                            </v-list-item>
                            <v-divider />
